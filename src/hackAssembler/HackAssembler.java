...
public class HackAssembler {
    ...
    public HackAssembler(String fileName, String inputDirectory, String outputDirectory, int n) throws IOException {
        if (n > 0){
            debugMode = true;
        }
        symbolTable = new SymbolTable(n);
        codeTable = new CodeTable(n);
        parser = new AssemblyFileParser(inputDirectory + fileName + ".asm", n);
        hackWriter = new FileWriter(outputDirectory + fileName + "\\hack\\" + fileName + ".hack");
        listWriter = new FileWriter(outputDirectory + fileName + "\\hack\\" + fileName + "_listing.txt");
    }
